...
def gconnect():
    ...
    print('Validate state token: {}'.format(login_session['state']))
    if request.args.get('state') != login_session['state']:
        response = make_response(json.dumps('Invalid state parameter'), 401)
        response.headers['Content-Type'] = 'application/json'
        return response
    ...
    print('Obtain authorization code: {}'.format(request.data))
    code = request.data
    try:
    ...
    print('Check that the access token is valid: {}'.format(credentials.access_token))
    ...
    print('Verify that the access token is used for the intended user: {}'.format(credentials.id_token['sub']))
    ...
    print('Verify that the access token is valid for this app: {}'.format(CLIENT_ID))
